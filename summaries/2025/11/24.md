...
## 11:26:48 PM
The provided code changes document the development of an "Integrity Handler" application in Go, primarily focused on securing filesystem directories through encryption using `fscrypt` and a Trusted Platform Module (TPM).

**Key Information from Changes:**

The Integrity Handler uses a File Encryption Master Key (FEMK) which is encrypted and stored at `/boot/integrity/femk.enc`. At runtime, this key is managed within a `vault.SecureContainer`, indicating a design for secure in-memory handling. The system relies on a specific directory structure for its operations:
*   `/boot/integrity`: For Integrity Handler's core files.
*   `/opt/128technology/integrity/.migration-store`: A temporary directory used during fscrypt initialization and data migration.
*   `/opt/128technology/integrity/migration-recovery`: A recovery directory for data in case of migration failures, often requiring manual intervention.
*   `/opt/128technology/integrity/manifest.d`: Where integrity manifest files are stored.

The application's lifecycle involves:
1.  **Environment Verification:** Checks if it's run as root, kernel version is at least 5.4, the filesystem supports encryption, the `fscrypt` command is available, and the TPM is initialized.
2.  **Directory Setup:** Ensures all necessary operational directories exist with appropriate permissions. It also attempts to clean the migration directory on startup.
3.  **Integrity Enablement:** This process, designed to be idempotent, involves resolving the FEMK, setting up `fscrypt` on the mount point, and then encrypting target directories specified in manifests. The encryption process temporarily moves contents out of a target directory, encrypts the now-empty directory, and then moves the contents back.
4.  **Integrity Unlock:** Unlocks directories that have been previously encrypted.

Robust error handling is a recurring theme, with functions returning detailed errors using `fmt.Errorf` and specific `ExitCode` values (e.g., `ExitIncompatible` for environment issues, `ExitCompromised` for integrity violations) for structured process exits. The `github.com/spf13/afero` library is used for filesystem operations, indicating an abstract approach for better testability or diverse storage backends.

**File-Specific Updates:**

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`:**
    *   **11/21/2025, 2:26:50 PM:** Initial definition of `const` variables for critical directory paths like `BootDirPath`, `EncryptedKeyPath`, `MigrationDirPath`, `RecoveryDirPath`, and `ManifestDirPath`. The package comment at this point describes a "secure container to hold our key at runtime."
    *   **11/21/2025, 4:02:40 PM:** The package comment was simplified from a detailed description of key container mechanisms to a more general "contains common globals." The constants themselves remained unchanged.
*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`:**
    *   **11/21/2025, 2:27:15 PM:** This file was introduced, laying out the core logic of the Integrity Handler. It includes functions for environment verification (`verifyAndInitializeEnvironment`), enabling integrity (`enableIntegrity`), unlocking encrypted directories (`unlockEncryptedDirs`), managing the filesystem key (`getFsKey`), ensuring directory existence (`ensureDirectoriesExist`), and handling recovery directory warnings (`handleRecoveryDirectoryWarnings`).
    *   **11/21/2025, 2:27:22 PM, 2:28:49 PM, 2:40:35 PM:** These timestamps show subsequent saves of the `integrity.go` file without any discernible functional code changes in the provided log snippets.
*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go` (11/21/2025, 4:00:45 PM):** This file defines the application's entry point. It sets up logging, parses command-line arguments (specifically a `mountPath`), handles versioning (using `//go:embed`), and orchestrates the main `run` function. The `run` function calls the core Integrity Handler logic, manages specific exit codes for different error scenarios (e.g., `ExitIncompatible`, `ExitCompromised`), and logs execution details.
*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go` (11/21/2025, 4:11:30 PM):** This file was introduced to encapsulate the `fscrypt`-specific logic. It defines various errors related to `fscrypt` operations, a `protectorName` constant ("FEMK"), and the `SetupOnMount` function for global and filesystem-level `fscrypt` configuration. Crucially, it introduces the `EncryptionResult` struct to track the outcome of encryption attempts and the `EncryptTargetDirs` and `encryptTargetDir` functions, detailing the multi-step process of vacating a directory, recreating it as encrypted, and then moving data back.

**Timestamps of Significant Changes:**

*   **11/21/2025, 2:26:50 PM:** Initial definition of critical directory paths.
*   **11/21/2025, 2:27:15 PM:** Introduction of the core Integrity Handler logic.
*   **11/21/2025, 4:00:45 PM:** Implementation of the application's main entry point and execution flow.
*   **11/21/2025, 4:11:30 PM:** Detailed implementation of `fscrypt` integration, including the multi-step directory encryption process.

**Patterns and Recurring Elements:**

*   **Juniper Networks Copyright:** All files include the copyright notice `// Copyright (c) Juniper Networks, Inc. 2025. All rights reserved.`.
*   **Security Focus:** A strong emphasis on secure operations, including key management via TPM, secure key containers, and filesystem encryption (`fscrypt`).
*   **Modular Design:** Code is organized into packages (`common`, `fscrypt`, `tpm`, `vault`) with clear responsibilities.
*   **Comprehensive Logging:** Extensive use of a custom `log` package (`log.Debug`, `log.Info`, `log.Warn`, `log.Error`) for tracing execution and issues.
*   **Error Handling Detail:** Consistent use of Go's error wrapping and checking (`errors.Is`, `errors.As`, `fmt.Errorf("%w: %w", ...)`) and custom error types for precise problem reporting.
*   **Idempotency Requirement:** Several key functions are explicitly designed to be idempotent to handle repeated calls without adverse effects.
*   **TODO Comments:** Indications of ongoing development or planned features, often related to incorporating more context or refining recovery mechanisms.