...
## 3:26:52 PM
The codebase primarily focuses on a Go application named "Integrity Handler" developed by Juniper Networks, Inc. in late November 2025, designed for securing file systems using encryption, likely involving TPM (Trusted Platform Module) and fscrypt.

### File-Specific Updates:

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **Initial state (11/21/2025, 2:26:50 PM):** Defines critical constant paths for the Integrity Handler, including `/boot/integrity` (BootDirPath), `/boot/integrity/femk.enc` (EncryptedKeyPath), `/opt/128technology/integrity/.migration-store` (MigrationDirPath), `/opt/128technology/integrity/migration-recovery` (RecoveryDirPath), and `/opt/128technology/integrity/manifest.d` (ManifestDirPath). The initial package comment was verbose, mentioning secure key containers and common variables.
    *   **Update (11/21/2025, 4:02:40 PM):** The package comment was simplified to "Package common contains common globals used throughout Integrity Handler." The defined path constants remained unchanged.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **Consistent state (11/21/2025, 2:27:15 PM - 11/21/2025, 2:40:35 PM):** This file contains the core logic for the Integrity Handler. Key functionalities include:
        *   `verifyAndInitializeEnvironment`: Ensures the system meets requirements such as being run as root, kernel version >= 5.4, filesystem encryption support, and TPM initialization.
        *   `enableIntegrity`: Manages the overall encryption process, including creating necessary directories, handling the File Encryption Master Key (FEMK), setting up `fscrypt`, and encrypting target directories. This function is explicitly noted to be idempotent.
        *   `unlockEncryptedDirs`: Responsible for unlocking encrypted directories.
        *   `getFsKey`: Securely loads and manages the filesystem encryption key, including wiping plaintext keys after use.
        *   `ensureDirectoriesExist`: Creates and cleans various integrity-related directories with specific permissions.
        *   `handleRecoveryDirectoryWarnings`: Checks for and logs warnings about unexpected contents in recovery directories.
    *   No functional changes were observed in this file across the multiple timestamps provided, suggesting intermediate saves or minor non-functional edits not captured in the log.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**
    *   **Initial and only update (11/21/2025, 4:00:45 PM):** This is the application's entry point. It defines `ExitCode` constants (e.g., `ExitSuccess`, `ExitCompromised`) that align with systemd service return codes. The `main` function parses a mount path argument, initializes logging, and orchestrates the calls to `handleRecoveryDirectoryWarnings`, `verifyAndInitializeEnvironment`, `enableIntegrity`, and `unlockEncryptedDirs`. It includes an embedded version string (`integrity-handler.version`).

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **Initial state (11/21/2025, 4:11:30 PM):** Provides high-level Go wrappers for `fscrypt` operations. It defines various specific error types (e.g., `ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`, `ErrFscryptNotSupportedOnMount`). The `SetupOnMount` function sets up global and filesystem-specific `fscrypt` configurations. The `EncryptionResult` struct tracks detailed outcomes of encryption operations (successes, already encrypted, and various failure types). The `encryptTargetDir` function outlines a multi-step process for encrypting a directory, involving relocating contents, recreating the directory, and then encrypting it.
    *   **Significant Update (11/25/2025, 4:04:34 PM):**
        *   The import for the `common` package was removed, indicating a step towards increased modularity and reduced inter-package dependencies.
        *   New specific error variables (`ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore`) were introduced to provide finer-grained error reporting.
        *   The `EncryptionResult` struct was refined by removing the `FailedToRecover` field, and corresponding methods (`newEncryptionResult`, `Failed`, `BuildError`) were updated to reflect this change.
        *   The `EncryptTargetDirs` function was refactored to directly manage temporary directory paths (though a `TODO` comment suggests further refinement for `migrationDirPath`) and to include a `switch` statement for improved error handling after calling `encryptTargetDir`. Successful encryption now explicitly triggers the removal of temporary migration directories.
        *   The `encryptTargetDir` function's signature was modified to accept `tempDir` as an argument, aligning with the updated `EncryptTargetDirs` logic.

### Timestamps of Significant Changes:

*   **11/21/2025, 2:26:50 PM:** Establishment of core directory paths for integrity handling.
*   **11/21/2025, 2:27:15 PM:** Initial comprehensive implementation of the `IntegrityHandler`'s primary logic.
*   **11/21/2025, 4:00:45 PM:** Introduction of the `main` application entry point, defining execution flow and exit codes.
*   **11/21/2025, 4:02:40 PM:** A minor, cosmetic adjustment to a package comment in `directories.go`.
*   **11/21/2025, 4:11:30 PM:** Initial implementation of `fscrypt` integration and wrapper functions.
*   **11/25/2025, 4:04:34 PM:** The latest and most substantial functional update, involving significant refactoring in `fscrypt.go` to improve modularity, error handling, and the encryption workflow.

### Patterns or Recurring Elements:

*   **Copyright:** All files include the "Copyright (c) Juniper Networks, Inc. 2025. All rights reserved." header, indicating a common origin and ownership.
*   **Security Focus:** The consistent use of terms like "Integrity Handler," "fscrypt," "TPM," "vault," "SecureContainer," and "FEMK" (File Encryption Master Key) highlights a core mission of securing the filesystem through encryption and integrity checks.
*   **Go Error Handling:** Standard Go error handling patterns, particularly `errors.New` and `fmt.Errorf("%w: %w", ...)` for error wrapping, are extensively used across the codebase for detailed and contextual error reporting.
*   **Idempotency:** Specific functions, such as `enableIntegrity` and `EncryptTargetDirs`, are explicitly designed and commented to be idempotent, ensuring consistent behavior on repeated calls.
*   **Temporary Directories:** The repeated use of specific directories like `/opt/128technology/integrity/.migration-store` and `/opt/128technology/integrity/migration-recovery` points to a design pattern involving staging areas for data migration during encryption and for recovery in case of failures.
*   **Logging:** The `log` package (likely `github.com/Juniper-SSN/ssr/go/src/log`) is consistently used for various levels of output, from debug messages to critical errors, aiding in monitoring and troubleshooting.
*   **Filesystem Abstraction:** The `github.com/spf13/afero` library is used for filesystem operations, which is a common pattern for testing and mocking filesystem interactions.

## 4:26:53 PM
The provided log details changes across the `Integrity Handler` Go application, focusing on file integrity, encryption, and secure key management.

### File-Specific Updates:

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **11/21/2025, 2:26:50 PM**: Initial commit defining core directory paths as constants: `/boot/integrity` (for handler files and encrypted keys), and `/opt/128technology/integrity/` subdirectories for migration store, recovery data, and manifest files.
    *   **11/21/2025, 4:02:40 PM**: A minor textual update to the package comment, changing it from a description of secure container memory to "Package common contains common globals used throughout Integrity Handler." The constants themselves remained unchanged.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **11/21/2025, 2:27:15 PM**, **2:27:22 PM**, **2:28:49 PM**, **2:40:35 PM**: This file, serving as the core logic for the Integrity Handler, remained stable across these four timestamps. It defines functions for:
        *   `verifyAndInitializeEnvironment`: Checks system requirements like root privileges, kernel version (>= 5.4), filesystem encryption support (via `fscrypt`), and TPM initialization.
        *   `enableIntegrity`: Orchestrates the encryption process by ensuring directories exist, resolving/creating a File Encryption Master Key (FEMK), setting up `fscrypt`, and encrypting target directories. It's noted to be idempotent.
        *   `unlockEncryptedDirs`: Unlocks previously encrypted directories.
        *   `getFsKey`: A lazy-loading function to retrieve and securely store the decrypted filesystem key using a `vault.SecureContainer`, including explicit memory wiping of the plaintext key.
        *   `ensureDirectoriesExist`: Creates and cleans essential operational directories with specific permissions.
        *   `handleRecoveryDirectoryWarnings`: Checks for data in the recovery directory and logs warnings if contents are found, indicating a potential issue requiring manual intervention.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**
    *   **11/21/2025, 4:00:45 PM**: This is the application's entry point, defining its main execution flow.
        ...
        *   The `run` function orchestrates the application's lifecycle: logging version, initializing application state, calling `handleRecoveryDirectoryWarnings`, `verifyAndInitializeEnvironment`, `enableIntegrity`, and `unlockEncryptedDirs`. It includes detailed error handling to return appropriate `ExitCode` values based on the outcome of integrity checks and encryption processes.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **11/21/2025, 4:11:30 PM**: Initial implementation of high-level wrappers around `fscrypt`.
        *   Introduces various `Err` constants for specific failure conditions (e.g., `ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`).
        *   `SetupOnMount`: Handles global `fscrypt` setup and filesystem-specific setup for a given mount point.
        *   `EncryptionResult`: A struct to track the outcome of encryption attempts (success, already encrypted, various failure types including `FailedToRecover`).
        *   `EncryptTargetDirs`: Iterates through target directories, checking their encryption status and calling `encryptTargetDir` for unencrypted ones.
        *   `encryptTargetDir`: This function performs the core encryption logic: relocating contents out of the target, recreating an empty target with specific permissions, encrypting it, and handling recovery in case of failures.
    *   **11/25/2025, 4:04:34 PM**: **Significant refactoring and enhancement.**
        *   **Error Reporting Refinement:** Added more granular error constants (`ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore`). The `EncryptionResult` struct and its error reporting methods were simplified by removing `FailedToRecover`, implying a revised approach to recovery scenarios.
        *   **Dependency Change:** The import of the `common` package was removed. Correspondingly, paths like `MigrationDirPath` are now either hardcoded within `EncryptTargetDirs` or passed as arguments to `encryptTargetDir`.
        *   **Encryption Flow Update:**
            *   `EncryptTargetDirs` now includes a `switch` statement to categorize errors returned by `encryptTargetDir` and explicitly cleans up the temporary migration directory upon successful encryption.
            *   The `encryptTargetDir` function's signature was altered to receive `tempDir` as an argument. Crucially, the internal logic for managing a `recoveryDir` and its associated error handling was removed, and the function now returns an `error` directly, allowing for better error propagation up the call stack.

### Timestamps of Significant Changes:

*   **11/21/2025, 2:27 PM - 2:40 PM**: Initial commits for the core `IntegrityHandler` (in `integrity.go`) and common directory definitions (`common/directories.go`).
*   **11/21/2025, 4:00 PM - 4:11 PM**: Introduction of the `main` application logic and the first iteration of `fscrypt` integration.
*   **11/25/2025, 4:04:34 PM**: A major refactoring of the `fscrypt` package, indicating significant development and refinement in the encryption and error recovery mechanisms. This timestamp marks the most substantial code change observed in the log.

### Patterns or Recurring Elements:

*   **Copyright Notices:** All code files include a `Juniper Networks, Inc. 2025` copyright notice.
*   **Security Focus:** The entire codebase is dedicated to "Integrity Handler" functionalities, centered on file encryption, TPM integration, and secure key management using a `vault.SecureContainer`. This is evident through terms like `FEMK` (File Encryption Master Key), `errIntegrityCompromised`, and secure memory handling.
*   **Robust Error Handling:** There is consistent and extensive use of Go's `errors` package features (e.g., `errors.New`, `fmt.Errorf`, `errors.Is`, `errors.As`, `errors.Join`) to create, wrap, and handle errors, providing detailed context for failures.
*   **Structured Logging:** The application utilizes a `log` package (from `github.com/Juniper-SSN/ssr/go/src/log`) with various levels (`Debug`, `Info`, `Warnf`, `Errorf`) for comprehensive tracing and reporting of application state and issues.
*   **Directory Management:** A recurring pattern involves creating and managing specific directories (`/boot/integrity`, `/opt/128technology/integrity/.migration-store`, `/opt/128technology/integrity/migration-recovery`, `/opt/128technology/integrity/manifest.d`) with carefully defined file permissions (e.g., `0o750`, `0o700`).
*   **Idempotency:** Several key functions, particularly `enableIntegrity` and `EncryptTargetDirs`, are explicitly designed and commented as "idempotent," ensuring they can be safely called multiple times without unintended side effects.
*   **Modular Design:** The codebase is organized into distinct Go packages (`common`, `integrity`, `main`, `fscrypt`, `tpm`, `vault`), promoting modularity and separation of concerns.
*   **TODO Comments:** Numerous `TODO` comments indicate ongoing development, planned features, or areas for improvement, such as "best effort shred," manifest processing, and refining error unwind steps.

## 5:26:46 PM
The log details development on an Integrity Handler application written in Go, focusing on filesystem encryption and integrity verification. Key updates and patterns include:

**File-Specific Updates:**

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**:
    *   **Timestamp: 11/21/2025, 2:26:50 PM**: Initial creation defining critical directory paths for the Integrity Handler, such as `/boot/integrity` (BootDirPath), `/boot/integrity/femk.enc` (EncryptedKeyPath for File Encryption Master Key), `/opt/128technology/integrity/.migration-store` (MigrationDirPath), `/opt/128technology/integrity/migration-recovery` (RecoveryDirPath), and `/opt/128technology/integrity/manifest.d` (ManifestDirPath).
    *   **Timestamp: 11/21/2025, 4:02:40 PM**: A minor update changed the package comment from a slightly verbose description to a more concise "Package common contains common globals used throughout Integrity Handler."

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**:
    *   **Timestamp: 11/21/2025, 2:27:15 PM**: This substantial entry lays out the core logic for the Integrity Handler. It includes functions for:
        *   `verifyAndInitializeEnvironment`: Checks system requirements like root privileges, kernel version (>= 5.4), filesystem encryption support (via fscrypt), fscrypt command availability, and TPM initialization.
        *   `enableIntegrity`: Orchestrates the encryption process, including ensuring directories exist, resolving the FEMK, setting up fscrypt, and encrypting target directories. It is explicitly stated to be idempotent.
        *   `unlockEncryptedDirs`: Handles unlocking previously encrypted directories. Failures here are considered integrity compromises.
        *   `getFsKey`: A lazy-loading mechanism for the filesystem key, wrapping it in a `vault.SecureContainer` and securely wiping the plain key.
        ...
        *   `handleRecoveryDirectoryWarnings`: Checks for data in the recovery directory, warning that manual intervention might be needed.
    *   **Timestamp: 11/21/2025, 2:27:22 PM & 11/21/2025, 2:40:35 PM**: No functional changes were observed in these entries; they appear to be identical to the preceding version, possibly representing save operations.
    *   **Timestamp: 11/21/2025, 2:28:49 PM**: A very minor refactoring in `handleRecoveryDirectoryWarnings` changed how the `afero.Exists` function was called, passing the existing `fs` variable instead of creating a new `afero.NewOsFs()` instance.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**:
    *   **Timestamp: 11/21/2025, 4:00:45 PM**: Introduced as the main entry point for the application. It defines custom `ExitCode` constants (Success, Failure, Incompatible, Compromised) for distinct application outcomes. The `main` function parses a `--mount` flag, calls the `run` function, and handles different exit codes based on the outcome. The `run` function orchestrates calls to `handleRecoveryDirectoryWarnings`, `verifyAndInitializeEnvironment`, `enableIntegrity`, and `unlockEncryptedDirs`. It also references `integrity-handler.version` embedded via `//go:embed`.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**:
    *   **Timestamp: 11/21/2025, 4:11:30 PM**: Initial implementation of high-level wrappers around the `fscrypt` library. It defines numerous error constants specific to encryption failures (e.g., `ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`). It includes `SetupOnMount` for fscrypt global and filesystem setup, and an `EncryptionResult` struct to detail the outcome of encryption attempts (success, already encrypted, various failure types). The `EncryptTargetDirs` function iterates through specified directories, checking if they are already encrypted, and calls `encryptTargetDir` to perform the actual relocation of contents, recreation of the directory, and encryption. The provided snippet for `encryptTargetDir` was incomplete, indicating ongoing development.
    *   **Timestamp: 11/25/2025, 4:04:34 PM**: Significant refinement of the `fscrypt` logic.
        *   New, more granular error constants like `ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore` were introduced.
        *   The `EncryptionResult` struct was updated, notably removing the `FailedToRecover` field and related logic.
        *   The `EncryptTargetDirs` function no longer imports the `common` package and hardcodes the `tempDir` path (`"/opt/128technology/integrity/.migration-store/" + dirName`), which is a departure from using the shared `common.MigrationDirPath` constant. It also added a `switch` statement to handle new specific error types from `encryptTargetDir` and explicitly removes the temporary migration directory on success.
        *   The `encryptTargetDir` function was also updated, removing previous recovery directory handling (`recoveryDir` and `migrateToRecoveryDir` calls). The error handling within this function was adjusted to align with the new, more specific error types.

**Timestamps of Significant Changes:**

*   **11/21/2025, 2:26:50 PM**: Defined core directory paths for the application.
*   **11/21/2025, 2:27:15 PM**: Initial implementation of the Integrity Handler's core business logic.
*   **11/21/2025, 4:00:45 PM**: Established the application's `main` entry point and command-line interface.
*   **11/21/2025, 4:11:30 PM**: Introduced the fscrypt integration layer with initial encryption functionality.
*   **11/25/2025, 4:04:34 PM**: Major refactoring and enhancement of fscrypt error handling, including a notable change in path management within `EncryptTargetDirs` by hardcoding a path.

**Patterns or Recurring Elements:**

*   **Security and Integrity Focus**: The code consistently emphasizes integrity, encryption using `fscrypt`, and secure key management, often involving TPM (Trusted Platform Module) and `vault.SecureContainer` for the File Encryption Master Key (FEMK).
*   **Detailed Error Handling**: A strong pattern of defining granular error types and structures (like `EncryptionResult` and custom `ExitCode`s) for precise reporting of operation outcomes and integrity violations.
*   **Directory Management**: Centralized definition and consistent use of specific directory paths (e.g., `/boot/integrity`, `/opt/128technology/integrity/.migration-store`) for various purposes, including boot files, encrypted keys, migration data, recovery data, and manifests.
*   **Idempotency**: The requirement for core functions like `enableIntegrity` and `EncryptTargetDirs` to be idempotent is explicitly stated.
*   **Copyright Notice**: All code files include a consistent "Copyright (c) Juniper Networks, Inc. 2025. All rights reserved." header.
*   **Go Language Features**: Extensive use of Go's `context` for managing request lifecycles, `errors.Is`/`errors.As` for robust error checking, `filepath` for path manipulation, and `afero` for filesystem operations.

## 6:26:46 PM
The log details recent development on an "Integrity Handler" Go application, focusing on filesystem encryption, key management, and system integrity.

**File-Specific Updates:**

*   **/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go:**
    *   **Initial state (11/21/2025, 2:26:50 PM):** This file defines constant paths essential for the Integrity Handler, including `BootDirPath`, `EncryptedKeyPath`, `MigrationDirPath`, `RecoveryDirPath`, and `ManifestDirPath`. The package comment initially described the `common` package as holding keys in a secure container using `mmap`'ed and `mlocked` memory, as well as common variables.
    *   **Later update (11/21/2025, 4:02:40 PM):** The package comment was updated to a more generic description: "Package common contains common globals used throughout Integrity Handler." The constant definitions themselves remained unchanged.

*   **/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go:**
    *   **Multiple timestamps (11/21/2025, 2:27:15 PM; 2:27:22 PM; 2:28:49 PM; 2:40:35 PM):** The content of this file remained consistent across these entries. It implements core Integrity Handler logic. Key functions include:
        *   `verifyAndInitializeEnvironment`: Checks system prerequisites such as running as root, kernel version (>= 5.4), filesystem encryption support (via `fscrypt`), `fscrypt` command availability, and TPM initialization.
        *   `enableIntegrity`: Orchestrates the encryption process by ensuring necessary directories exist, resolving/creating a Filesystem Encryption Master Key (FEMK), obtaining a secure key from a vault, setting up `fscrypt` on the mount, and encrypting target directories.
        *   `unlockEncryptedDirs`: Handles unlocking previously encrypted directories, flagging integrity compromises on failure.
        *   `getFsKey`: Retrieves the filesystem key by decrypting the FEMK and securely storing it in a `vault.SecureContainer`, then safely wiping the plaintext key from memory.
        *   `ensureDirectoriesExist`: Creates and manages various application-specific directories, including cleaning up migration paths.
        *   `handleRecoveryDirectoryWarnings`: Logs warnings if data is found in recovery directories, indicating potential issues requiring manual intervention.

*   **/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go:**
    *   **Single update (11/21/2025, 4:00:45 PM):** This file contains the main entry point for the Integrity Handler application.
        *   It defines custom `ExitCode` constants for different application outcomes (success, generic failure, incompatibility, integrity compromised).
        *   It embeds the application version using `//go:embed integrity-handler.version`.
        *   The `main` function sets up logging, parses command-line arguments (specifically a `mount` path), and calls the `run` function to execute the main logic.
        *   The `run` function coordinates the verification, initialization, encryption, and unlocking phases, handling errors by returning specific `ExitCode` values, prominently `ExitCompromised` for integrity violations.

*   **/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go:**
    *   **Initial state (11/21/2025, 4:11:30 PM):** This file provides high-level wrappers for `fscrypt` operations. It defines numerous error types related to encryption and relocation failures, sets a `protectorName` constant ("FEMK"), and includes functions for setting up `fscrypt` on a mount, encrypting target directories (`EncryptTargetDirs`), and the detailed logic for encrypting a single directory (`encryptTargetDir`). It also defines an `EncryptionResult` struct to track outcomes.
    *   **Significant update (11/25/2025, 4:04:34 PM):** This update reflects a refactoring and enhancement of the `fscrypt` functionality:
        *   The import of the `common` package was removed, indicating a change in how common paths (like migration directories) are accessed or provided.
        *   New top-level error variables (`ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore`) were introduced for clearer error identification.
        *   The `EncryptionResult` struct was updated, notably removing the `FailedToRecover` field and adjusting related `Failed()` and `BuildError()` methods.
        *   The `EncryptTargetDirs` function was refactored to explicitly use a hardcoded migration path (`/opt/128technology/integrity/.migration-store/`) and now includes a `switch` statement for specific error handling after attempting to encrypt a directory, also adding logic to clean up temporary migration directories upon success.
        *   The signature of `encryptTargetDir` was modified to explicitly pass the `tempDir` and streamline its recovery handling, with the `recoveryDir` parameter removed.

**Timestamps of Significant Changes:**

*   **11/21/2025, 2:26:50 PM:** Initial commit of `common/directories.go` defining key paths.
*   **11/21/2025, 2:27:15 PM:** Initial commit of `integrity.go` showing core application logic.
*   **11/21/2025, 4:00:45 PM:** `main.go` was updated, establishing the application's entry point, argument parsing, and overall flow for integrity handling, including specific exit codes.
*   **11/21/2025, 4:02:40 PM:** A minor but notable change to `common/directories.go` involved updating its package comment for clarity, removing specific details about key storage mechanisms.
*   **11/21/2025, 4:11:30 PM:** Initial commit of `fscrypt/fscrypt.go`, introducing the high-level API for filesystem encryption.
*   **11/25/2025, 4:04:34 PM:** A substantial refactoring of `fscrypt/fscrypt.go`, including removal of the `common` import, new specific error types, and revised error handling and directory management within the encryption process.

**Patterns and Recurring Elements:**

*   **Copyright Notice:** All Go files consistently bear the "Copyright (c) Juniper Networks, Inc. 2025. All rights reserved." notice.
*   **Integrity and Security Focus:** The project name "Integrity Handler" and the consistent use of `fscrypt`, TPM initialization, and `vault.SecureContainer` underscore a strong emphasis on maintaining system integrity and secure handling of sensitive keys (FEMK).
*   **Error Handling:** The codebase extensively uses Go's error handling patterns, including `errors.New`, `fmt.Errorf`, `errors.Is`, `errors.As`, and `errors.Join`, and defines custom error types for specific failure scenarios.
*   **Filesystem Abstraction:** `github.com/spf13/afero` is used across files (`integrity.go`, `fscrypt/fscrypt.go`) to abstract filesystem operations, making the code more testable and flexible.
*   **Structured Logging:** The application uses `github.com/Juniper-SSN/ssr/go/src/log` for consistent logging at various levels (Debug, Info, Warnf, Errorf).
*   **Idempotency:** Both `enableIntegrity` and `EncryptTargetDirs` functions are explicitly designed to be idempotent, meaning they can be run multiple times without causing unintended side effects.
*   **Refactoring Trend:** The changes in `fscrypt.go` from 11/21 to 11/25, particularly the removal of the `common` import and explicit error handling, suggest a move towards more modular and encapsulated design within the `fscrypt` package. The multiple identical entries for `integrity.go` on 11/21 indicate a period of minor, internal changes or frequent saves without significant functional diffs captured in the provided log.

## 7:26:51 PM
The provided log details changes across several Go source files within an "Integrity Handler" application, primarily focusing on filesystem encryption and integrity management.

**File-Specific Updates and Timestamps:**

1.  **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **11/21/2025, 2:26:50 PM:** This file was initially created, defining various constant paths critical to the Integrity Handler's operation. These include `BootDirPath` (`/boot/integrity`), `EncryptedKeyPath` (`/boot/integrity/femk.enc`), `MigrationDirPath` (`/opt/128technology/integrity/.migration-store`), `RecoveryDirPath` (`/opt/128technology/integrity/migration-recovery`), and `ManifestDirPath` (`/opt/128technology/integrity/manifest.d`). The package comment indicated its role in holding secure keys at runtime and common variables.
    *   **11/21/2025, 4:02:40 PM:** A minor change was made to the package comment, clarifying its purpose as containing "common globals used throughout Integrity Handler." The defined constants remained unchanged.

2.  **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **11/21/2025, 2:27:15 PM:** This file outlines the core functionality of the Integrity Handler. It includes:
        *   `verifyAndInitializeEnvironment`: Checks system requirements such as root privileges, kernel version (>= 5.4), filesystem encryption support (using `fscrypt/metadata`), `fscrypt` command availability, and TPM initialization.
        *   `enableIntegrity`: Manages the encryption process, ensuring required directories exist, resolving/creating the File Encryption Master Key (FEMK), setting up `fscrypt`, and encrypting target directories.
        *   `unlockEncryptedDirs`: Handles unlocking previously encrypted directories.
        *   `getFsKey`: Securely retrieves the filesystem key from a vault and wipes the plaintext key from memory.
        *   `ensureDirectoriesExist`: Creates and, in the case of migration directories, cleans various application-specific directories with appropriate permissions.
        *   `handleRecoveryDirectoryWarnings`: Logs warnings if data is found in the recovery directory, suggesting manual intervention.
    *   **11/21/2025, 2:27:22 PM, 2:28:49 PM, 2:40:35 PM:** No functional or content changes were recorded in the provided log for these timestamps in this file.

3.  **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**
    *   **11/21/2025, 4:00:45 PM:** This serves as the application's entry point. It sets up logging, parses command-line arguments (specifically a `mount` path), and defines `ExitCode` constants for different application outcomes: `ExitSuccess` (0), `ExitFailure` (1), `ExitIncompatible` (6), and `ExitCompromised` (78). The `run` function orchestrates the main application flow, calling the environment verification, integrity enablement, and directory unlocking functions, and mapping their results to the defined exit codes, particularly marking integrity violations with `ExitCompromised`. It also embeds the application version string.

4.  **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **11/21/2025, 4:11:30 PM:** This file introduces a `fscrypt` wrapper package with functions to interface with `fscrypt` internals. It defines various granular error types related to encryption operations (e.g., `ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`, `ErrFscryptNotSupportedOnMount`). Key components include:
        *   `SetupOnMount`: Performs global and mount-specific `fscrypt` setup, designed to be idempotent.
        *   `EncryptionResult`: A struct to track the success and different categories of failures during the encryption process.
        *   `EncryptTargetDirs`: Iterates through a list of target directories, checking if they are already encrypted before attempting to encrypt them individually.
        *   `encryptTargetDir`: Contains the core logic for encrypting a single directory, involving temporarily moving its contents, recreating the directory as empty, encrypting it, and preparing to restore contents. It initially relies on paths from the `common` package.
    *   **11/25/2025, 4:04:34 PM:** This timestamp marks significant refactoring:
        *   **Dependency Removal:** The import of `github.com/Juniper-SSN/ssr/go/bin/IntegrityHandler/common` was removed.
        *   **Enhanced Error Specificity:** New, more specific error types (`ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore`) were introduced.
        *   **`EncryptionResult` Update:** The `FailedToRecover` field and related logic were removed from the `EncryptionResult` struct and its associated methods (`newEncryptionResult`, `Failed`, `BuildError`).
        *   **Refined Encryption Orchestration:** The `EncryptTargetDirs` function was updated to use a `switch` statement to handle specific errors returned by `encryptTargetDir`, providing more detailed logging for different failure scenarios. It also started hardcoding the `tempDir` path directly within the function (e.g., `"/opt/128technology/integrity/.migration-store/" + dirName`) instead of relying on the `common` package.
        *   **`encryptTargetDir` Signature Change:** The `encryptTargetDir` function's signature was modified to accept `tempDir` as an explicit argument, rather than calculating it internally, reflecting the removal of the `common` package dependency for path definitions within this function. The provided snippet for this function is incomplete.

**Patterns and Recurring Elements:**

*   **Copyright and Future-Dating:** All files are copyrighted to "Juniper Networks, Inc. 2025," consistent with the timestamps provided in the log.
*   **Security and Integrity Focus:** The overarching theme is robust system integrity, leveraging filesystem encryption (`fscrypt`), hardware security modules (TPM), and secure key management (`vault`). Many error messages specifically refer to "integrity compromised."
*   **Error Handling Practices:** The codebase demonstrates comprehensive error handling, utilizing `errors.New`, `fmt.Errorf`, `errors.Is`, and `errors.Join` to create distinct and aggregable error types for better diagnostics.
*   **Idempotency:** Key functions like `enableIntegrity` and `EncryptTargetDirs` are explicitly noted as requiring idempotency, crucial for reliable system operations and recovery.
*   **Logging:** Consistent use of a custom `log` package (`github.com/Juniper-SSN/ssr/go/src/log`) for debugging, informational, warning, and error messages throughout the application.
*   **Filesystem Abstraction:** The `github.com/spf13/afero` library is used for filesystem operations, which is a common pattern for creating testable and flexible file system interactions.
*   **Directory Structure for Operations:** The consistent definition and use of dedicated directories for boot, encrypted keys, migration, recovery, and manifests (`/boot/integrity`, `/opt/128technology/integrity/.migration-store`, etc.) indicate a structured approach to managing sensitive and operational data.
*   **Refinement of Dependencies and Error Handling:** The changes in `fscrypt/fscrypt.go` demonstrate an ongoing refinement towards more granular error reporting and potentially a move to decouple path definitions from a generic `common` package, suggesting increased modularity and explicit dependency management.

## 8:26:52 PM
The provided log details changes to the "Integrity Handler" Go application, focusing on system integrity and encryption features.

**File-Specific Updates and Timestamps:**

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **Timestamp: 11/21/2025, 2:26:50 PM:** This file was initially introduced, defining constants for critical directory paths used by the Integrity Handler. These paths include `/boot/integrity` (for handler files), `/boot/integrity/femk.enc` (for the encrypted Front-End Master Key), `/opt/128technology/integrity/.migration-store` (for migration data), `/opt/128technology/integrity/migration-recovery` (for recovery data), and `/opt/128technology/integrity/manifest.d` (for integrity manifest files). The package comment described its role in holding secure keys at runtime.
    *   **Timestamp: 11/21/2025, 4:02:40 PM:** The package comment was updated to a more concise `Package common contains common globals used throughout Integrity Handler.`, while the directory constants themselves remained unchanged.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **Timestamps: 11/21/2025, 2:27:15 PM to 11/21/2025, 2:40:35 PM:** Across multiple entries within this timeframe, the content of this file appears consistent, indicating no functional changes were made in this specific sequence. This file contains the core logic for the Integrity Handler application. Key functions include:
        *   `verifyAndInitializeEnvironment`: Checks system prerequisites such as running as root, kernel version (>= 5.4), filesystem support for encryption (using `fscrypt`), and TPM initialization.
        *   `enableIntegrity`: Orchestrates the encryption process by ensuring directories exist, resolving/decrypting the FEMK, setting up `fscrypt`, and invoking `EncryptTargetDirs`.
        *   `getFsKey`: Securely decrypts and loads the filesystem key into a `vault.SecureContainer`, then securely wipes the plain key.
        *   `ensureDirectoriesExist`: Creates and, in some cases, cleans up the specific directories defined in the `common` package (Migration, Recovery, Manifest, Boot).
        *   `handleRecoveryDirectoryWarnings`: Logs warnings if unexpected contents are found in the recovery directory.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**
    *   **Timestamp: 11/21/2025, 4:00:45 PM:** This new file was introduced as the main entry point for the application. It handles command-line arguments (e.g., `mountPath`), sets up logging, embeds the application version, and defines custom `ExitCode` values (e.g., `ExitSuccess`, `ExitCompromised`, `ExitIncompatible`). The `run` function orchestrates the application's flow, calling environment verification, directory warning handling, `enableIntegrity`, and `unlockEncryptedDirs`.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **Timestamp: 11/21/2025, 4:11:30 PM:** This file was introduced, providing high-level wrappers for `fscrypt` operations. It defined specific error types for encryption failures, the `protectorName` "FEMK", and `SetupOnMount` for global `fscrypt` configuration. The `EncryptionResult` struct tracked the outcomes of encryption (successes, already encrypted, and various failure categories including `FailedToRecover`). The `EncryptTargetDirs` function coordinated the encryption of multiple directories by relocating contents, recreating directories, and encrypting them.
    *   **Timestamp: 11/25/2025, 4:04:34 PM:** Significant refactoring occurred in this file:
        *   The import of the `common` package was removed. Consequently, references to `common.MigrationDirPath` were replaced with a hardcoded path `"/opt/128technology/integrity/.migration-store/"`.
        *   New, explicit error constants (`ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore`) were introduced for more granular error reporting.
        *   The `EncryptionResult` struct was simplified by removing the `FailedToRecover` slice, and its associated `Failed()` and `BuildError()` methods were updated accordingly.
        *   The `EncryptTargetDirs` function's error handling for `encryptTargetDir` was redesigned to use a `switch` statement over specific error returns, suggesting `encryptTargetDir` now returns errors directly instead of modifying a result struct passed by reference. A new cleanup step was added to remove temporary directories on successful encryption.
        *   The signature of `encryptTargetDir` was changed to accept `tempDir string` and return an `error`, promoting clearer error propagation.

**Patterns and Recurring Elements:**

*   **Security and Integrity Focus:** The dominant theme is maintaining system integrity through filesystem encryption, leveraging `fscrypt`, TPM integration, and secure key handling (e.g., `FEMK`, `vault.SecureContainer`). The application includes checks for "integrity compromised" states.
*   **Idempotency:** Key functions like `enableIntegrity` and `EncryptTargetDirs` are explicitly designed to be idempotent, ensuring consistent behavior even if called multiple times.
*   **Robust Error Handling:** The codebase consistently defines and uses specific error types, with a progression towards more granular and explicit error reporting and propagation (seen in the `fscrypt.go` changes). `errors.Join` is used to aggregate multiple errors.
*   **Directory Standardization:** A consistent set of dedicated directories under `/boot/integrity` and `/opt/128technology/integrity/` are used for specific purposes, such as key storage, migration, recovery, and manifests.
*   **External Library Reliance:** The use of `github.com/google/fscrypt` for encryption mechanics and `github.com/spf13/afero` for abstracting filesystem operations is prevalent across core files.
*   **Version and Copyright:** All files include a "Copyright (c) Juniper Networks, Inc. 2025. All rights reserved." header, and the `main.go` file embeds the application version.
*   **Code Refactoring for Clarity:** The changes in `fscrypt.go` indicate an ongoing effort to improve modularity, simplify function signatures, and enhance the clarity of error handling and dependency management within specific packages.
*   **Development Timeline:** Initial core components and application entry point were established on `11/21/2025`, with significant refactoring and refinement of the `fscrypt` encryption logic occurring a few days later on `11/25/2025`.

## 9:26:48 PM
The provided logs detail a series of changes to an "Integrity Handler" Go application, primarily focused on secure file system encryption using `fscrypt` and TPM/vTPM integration.

**File-Specific Updates:**

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **Timestamp: 11/21/2025, 2:26:50 PM:** This file was initially defined, establishing constants for key directory paths related to the Integrity Handler. These paths include `/boot/integrity` for core files, `/boot/integrity/femk.enc` for the encrypted File Encryption Master Key (FEMK), and `/opt/128technology/integrity/` for migration, recovery, and manifest data.
    *   **Timestamp: 11/21/2025, 4:02:40 PM:** The package comment was updated from a detailed description of secure key containers to a more concise `Package common contains common globals used throughout Integrity Handler.`, better reflecting the file's direct content. The defined directory constants remained unchanged.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **Timestamp: 11/21/2025, 2:27:15 PM:** The core logic for the Integrity Handler was introduced. This includes:
        *   Functions to `verifyAndInitializeEnvironment`, checking for root privileges, kernel version (>= 5.4), filesystem encryption support (via `fscrypt/metadata`), and TPM initialization.
        *   `enableIntegrity` and `unlockEncryptedDirs` functions, which orchestrate the encryption and decryption of target directories using a FEMK, `fscrypt`, and a `vault.SecureContainer`.
        *   `getFsKey` for lazy-loading and securely handling the filesystem key.
        *   `ensureDirectoriesExist` to create and clean up necessary operational directories with specific permissions.
        *   `handleRecoveryDirectoryWarnings` to alert on existing data in recovery directories.
    *   **Timestamps: 11/21/2025, 2:27:22 PM; 11/21/2025, 2:28:49 PM; 11/21/2025, 2:40:35 PM:** No functional changes were observed in these entries; they appear to be minor saves or reformatting without altering the code logic.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**
    *   **Timestamp: 11/21/2025, 4:00:45 PM:** This file was added, providing the main entry point for the application. It defines standard `ExitCode` constants (Success, Failure, Incompatible, Compromised) for systemd integration, embeds a version string, parses a `--mount` path argument, and orchestrates the `run` function. The `run` function calls the core integrity functions (`verifyAndInitializeEnvironment`, `enableIntegrity`, `unlockEncryptedDirs`) and handles their return values to determine the application's exit code.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **Timestamp: 11/21/2025, 4:11:30 PM:** The `fscrypt` package was introduced, containing high-level wrappers for `fscrypt` operations. Key components include:
        *   Definition of various specific error types related to fscrypt operations (e.g., `ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`).
        *   `SetupOnMount` to configure fscrypt globally and on specific mount points.
        *   `EncryptionResult` struct to detail the outcomes (success, already encrypted, various failures) of encryption attempts.
        *   `EncryptTargetDirs` function, designed to be idempotent, which iterates and calls `encryptTargetDir` for individual paths.
        *   `encryptTargetDir` which moves directory contents to a temporary location, encrypts the now-empty target directory, and then moves contents back.
    *   **Timestamp: 11/25/2025, 4:04:34 PM:** Significant refinements were made to the `fscrypt` package:
        *   Removed the import of `common` package, and hardcoded `/opt/128technology/integrity/.migration-store/` for `tempDir`, simplifying dependency.
        *   Introduced new, explicit error variables (`ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore`) to replace more general error handling, improving error specificity.
        *   The `EncryptionResult` struct and its related methods (`newEncryptionResult`, `Failed`, `BuildError`) were modified to remove tracking for `FailedToRecover` directories, suggesting a change in the recovery strategy or a pending implementation.
        *   `EncryptTargetDirs` was updated to use a `switch` statement for better categorization and debugging of different failure types, and to perform cleanup of temporary migration directories upon successful encryption.
        *   The `encryptTargetDir` function saw the removal of the `recoveryDir` variable and its associated logic, further indicating a revised approach to handling data recovery during encryption failures.

**Timestamps of Significant Changes:**

*   **11/21/2025, 2:26:50 PM:** Initial directory path definitions.
*   **11/21/2025, 2:27:15 PM:** Core Integrity Handler logic implementation.
*   **11/21/2025, 4:00:45 PM:** Introduction of the `main` application entry point and orchestration.
*   **11/21/2025, 4:02:40 PM:** Refinement of the package comment in `common/directories.go`.
*   **11/21/2025, 4:11:30 PM:** Initial implementation of the `fscrypt` wrapper package.
*   **11/25/2025, 4:04:34 PM:** Major refactoring and enhancement of error handling and recovery logic within `fscrypt.go`.

**Patterns and Recurring Elements:**

*   **Copyright:** All files consistently include the copyright notice `// Copyright (c) Juniper Networks, Inc. 2025. All rights reserved.`.
*   **Modularity:** The code is structured into distinct Go packages (`main`, `common`, `fscrypt`, `tpm`, `vault`) for different concerns.
*   **Security Focus:** A strong emphasis on integrity and security is evident through the use of `fscrypt` for encryption, TPM/vTPM for key management, and `vault.SecureContainer` for in-memory key protection. The `IntegrityHandler` itself points to this core concern.
*   **Robust Error Handling:** Extensive use of Go's `errors` package, including `errors.Join` and `errors.Is`, for detailed error reporting and categorized failures, often wrapped with `fmt.Errorf`.
*   **Logging:** A custom `log` package (`github.com/Juniper-SSN/ssr/go/src/log`) is used consistently across files for debugging, informational, warning, and error messages.
*   **Idempotency:** Functions like `enableIntegrity` and `EncryptTargetDirs` are explicitly designed and commented to be idempotent, ensuring repeated calls do not cause adverse effects.
*   **Directory Management:** Specific directories (`/boot/integrity`, `/opt/128technology/integrity/.migration-store`, `/opt/128technology/integrity/migration-recovery`, `/opt/128technology/integrity/manifest.d`) are managed for operational data, temporary storage, and recovery purposes.
*   **Version Control:** The `main` package uses `//go:embed` to include a `integrity-handler.version` string, indicating standard versioning practices.
*   **Development Timeline:** The timestamps suggest an initial burst of development activity on 11/21/2025, setting up the core structure and initial functionality, followed by a significant refinement in the `fscrypt` package a few days later on 11/25/2025.

## 10:26:49 PM
The provided log details changes across several Go files related to an "Integrity Handler" application by Juniper Networks, focused on securing directories using fscrypt.

**File-Specific Updates and Timestamps:**

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **11/21/2025, 2:26:50 PM:** This file was initially introduced, defining constants for critical directory paths used by the Integrity Handler. These include `/boot/integrity` (`BootDirPath`), `/boot/integrity/femk.enc` (`EncryptedKeyPath`), `/opt/128technology/integrity/.migration-store` (`MigrationDirPath`), `/opt/128technology/integrity/migration-recovery` (`RecoveryDirPath`), and `/opt/128technology/integrity/manifest.d` (`ManifestDirPath`). The package comment originally referenced a secure container for keys.
    *   **11/21/2025, 4:02:40 PM:** A minor cosmetic change occurred, updating the package comment from a specific description about key containers to a more general statement: "Package common contains common globals used throughout Integrity Handler." No functional changes were observed.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **11/21/2025, 2:27:15 PM:** This appears to be the initial significant commit for the core `integrity.go` logic. It establishes functions for:
        *   `verifyAndInitializeEnvironment`: Checks system prerequisites like root privileges, kernel version (>= 5.4), filesystem encryption support (via `fscrypt/metadata.CheckSupport`), `fscrypt` command availability, and TPM initialization.
        *   `enableIntegrity`: Orchestrates the encryption process, including ensuring necessary directories exist, resolving/creating a File Encryption Master Key (FEMK), setting up fscrypt, and encrypting target directories.
        *   `unlockEncryptedDirs`: Handles unlocking previously encrypted directories.
        *   `getFsKey`: Manages the secure loading and wiping of the filesystem key using a `vault.SecureContainer`.
        *   `ensureDirectoriesExist`: Creates the various integrity-related directories defined in `common/directories.go` and cleans the migration directory.
        *   `handleRecoveryDirectoryWarnings`: Checks the recovery directory for any lingering contents, indicating potential past failures requiring manual intervention.
    *   **11/21/2025, 2:27:22 PM**, **2:28:49 PM**, **2:40:35 PM:** No functional changes were observed in `integrity.go` across these timestamps; the code content remained identical to the 2:27:15 PM entry. This suggests a series of saves or minor, unlogged non-functional edits.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**
    *   **11/21/2025, 4:00:45 PM:** This file was introduced as the main entry point for the Integrity Handler application. Key features include:
        *   Defining `ExitCode` constants (`ExitSuccess`, `ExitFailure`, `ExitIncompatible`, `ExitCompromised`) that align with systemd service return codes for clear status reporting.
        *   Embedding the application version using `//go:embed integrity-handler.version`.
        *   The `main` function parses a `mount` path argument.
        *   The `run` function orchestrates the application's lifecycle, calling `handleRecoveryDirectoryWarnings`, `verifyAndInitializeEnvironment`, `enableIntegrity`, and `unlockEncryptedDirs`. It includes comprehensive error handling to return the appropriate `ExitCode` based on the type of failure (e.g., `ExitIncompatible` for environment issues, `ExitCompromised` for integrity violations).

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **11/21/2025, 4:11:30 PM:** This file was initially added, providing high-level wrappers for fscrypt operations. It defines:
        *   Numerous `Err` constants for specific failure scenarios (e.g., `ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`, `ErrFscryptNotSupportedOnMount`).
        *   A `protectorName` constant set to "FEMK".
        *   `SetupOnMount`: A function to perform global and filesystem-specific fscrypt setup.
        *   `EncryptionResult` struct: A detailed structure to track the success and various types of failures during the encryption process (e.g., `Success`, `AlreadyEncrypted`, `FailedToRelocate`, `FailedToEncrypt`, `FailedToUnlock`, `FailedToRestore`, `FailedToRecover`).
        *   `EncryptTargetDirs`: The main function to encrypt target directories, handling idempotency and calling `encryptTargetDir`.
        *   `encryptTargetDir` (partial): Implements the core logic for migrating directory contents, recreating the directory, encrypting it, and handling recovery in case of failures. It directly used paths from the `common` package (e.g., `common.MigrationDirPath`, `common.RecoveryDirPath`).
    *   **11/25/2025, 4:04:34 PM:** A significant update occurred after a few days of development:
        *   **Refactoring:** The `github.com/Juniper-SSN/ssr/go/bin/IntegrityHandler/common` import was removed from this file, indicating a refactoring of how common directory paths are accessed or managed within the fscrypt package.
        *   **Enhanced Error Handling:** New specific error constants `ErrFailedToEncrypt`, `ErrFailedToUnlock`, and `ErrFailedToRestore` were introduced, providing more granular failure reporting.
        *   **Simplified `EncryptionResult`:** The `FailedToRecover` slice was removed from the `EncryptionResult` struct, and its usage was removed from `newEncryptionResult`, `Failed`, and `BuildError` methods.
        *   **`EncryptTargetDirs` Improvements:** This function was updated to replace the direct use of `common.MigrationDirPath` with a hardcoded `"/opt/128technology/integrity/.migration-store/"` for temporary directories, and includes a `switch` statement for more explicit error handling and cleanup of temporary migration directories upon success.
        *   The `encryptTargetDir` function was also updated, reflecting the removal of the `common` import, which implies changes to how it handles migration and recovery directory paths.

**Patterns and Recurring Elements:**

*   **Copyright Notice:** All code snippets consistently include the copyright notice: `// Copyright (c) Juniper Networks, Inc. 2025. All rights reserved.`.
*   **Juniper SSN Moniker:** The `github.com/Juniper-SSN/ssr/go/src/log` package is a consistent dependency for logging across the application. Other internal packages like `tpm`, `vault`, `fscrypt`, and `common` also fall under the `github.com/Juniper-SSN/ssr/go/bin/IntegrityHandler` namespace.
*   **Fscrypt Integration:** The core purpose of the application is deeply tied to `github.com/google/fscrypt` for filesystem encryption, with custom wrappers and error handling built around it.
*   **Secure Key Management:** The concept of a "secure container" and FEMK (File Encryption Master Key) is central, leveraging `vault` for key security and TPM for initialization.
*   **Robustness and Error Handling:** The code emphasizes idempotent operations, detailed error reporting with specific error types, and recovery mechanisms for data migration failures during encryption. The `EncryptionResult` struct and custom `ExitCode` values highlight this focus.
*   **Directory Management:** Specific directories for boot files, encrypted keys, migration data, recovery data, and manifest files (`/boot/integrity`, `/opt/128technology/integrity/.migration-store`, `/opt/128technology/integrity/migration-recovery`, `/opt/128technology/integrity/manifest.d`) are consistently used and managed.
*   **Development Progression:** The timestamps indicate an initial burst of activity defining core components (`directories.go`, `integrity.go`, `main.go`), followed by a period (several days) focusing on refining the complex fscrypt wrapper logic (`fscrypt.go`) to improve error handling and potentially refactor dependencies.

## 11:26:47 PM
This log details changes across several Go files within an "Integrity Handler" application, focusing on filesystem encryption and integrity management.

### File-Specific Updates:

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **Timestamp**: 11/21/2025, 2:26:50 PM and 11/21/2025, 4:02:40 PM.
    *   **Summary**: This file defines constant string paths used by the Integrity Handler. Key paths include `/boot/integrity` for handler files, `/boot/integrity/femk.enc` for the encrypted File Encryption Master Key (FEMK), `/opt/128technology/integrity/.migration-store` for migration data, `/opt/128technology/integrity/migration-recovery` for recovery data, and `/opt/128technology/integrity/manifest.d` for integrity manifests. The only change observed between the timestamps is a minor rewording of the package-level comment, clarifying its purpose as containing "common globals used throughout Integrity Handler" instead of a more specific mention of secure key containers. The directory constants themselves remained unchanged.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **Timestamps**: 11/21/2025, 2:27:15 PM; 11/21/2025, 2:27:22 PM; 11/21/2025, 2:28:49 PM; 11/21/2025, 2:40:35 PM.
    *   **Summary**: Across these four timestamps, the content of this file appears identical, suggesting no functional changes or only minor whitespace/comment updates not visible in the provided diffs. This file implements the core logic for the Integrity Handler. It includes functions to:
        *   `verifyAndInitializeEnvironment`: Check system requirements such as being run as root, kernel version (>= 5.4), filesystem encryption support (via `fscrypt`), and TPM initialization.
        *   `enableIntegrity`: Orchestrates the encryption process, including ensuring necessary directories exist, resolving the FEMK, setting up `fscrypt`, and encrypting target directories. It is explicitly noted as needing to be idempotent.
        *   `unlockEncryptedDirs`: Unlocks previously encrypted directories.
        *   `getFsKey`: Retrieves the filesystem key, decrypting the FEMK and storing it in a `vault.SecureContainer`, securely wiping the plaintext key afterwards.
        *   `ensureDirectoriesExist`: Creates all required application directories with appropriate permissions, including cleaning up the migration directory.
        *   `handleRecoveryDirectoryWarnings`: Checks for data in the recovery directory and logs warnings if found, indicating past failures.
        *   The file defines `errIntegrityCompromised` and imports packages for `fscrypt`, `tpm` (Trusted Platform Module), `vault` (secure key storage), `common` (for directory constants), `afero` (filesystem abstraction), and `log`.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/main.go`**
    *   **Timestamp**: 11/21/2025, 4:00:45 PM.
    *   **Summary**: This file serves as the entry point for the Integrity Handler application. It defines `ExitCode` constants for various application outcomes (Success, Failure, Incompatible Environment, Compromised Integrity). The `main` function initializes logging, parses a `mountPath` argument (defaulting to `/`), and calls the `run` function. The `run` function logs the application version, initializes the application state, calls `handleRecoveryDirectoryWarnings`, performs environment verification, and then proceeds to `enableIntegrity` and `unlockEncryptedDirs` for a set of `DefaultManifest()` targets. It maps different error types (especially `errIntegrityCompromised`) to specific exit codes for systemd compatibility.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **Timestamps**: 11/21/2025, 4:11:30 PM and 11/25/2025, 4:04:34 PM.
    *   **Summary**: This file provides high-level wrappers for `fscrypt` operations.
        *   **Initial state (11/21/2025, 4:11:30 PM)**: Defined several error types related to fscrypt operations (e.g., `ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`). It included an `EncryptionResult` struct to track success and various failure states, including `FailedToRecover`. The `encryptTargetDir` function internally constructed `tempDir` and `recoveryDir` using constants from the `common` package. It included a `migrateToRecoveryDir` call for failure scenarios.
        *   **Later state (11/25/2025, 4:04:34 PM)**: Significant refactoring occurred:
            *   The `github.com/Juniper-SSN/ssr/go/bin/IntegrityHandler/common` import was *removed*.
            *   New, more specific error variables (`ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore`) were introduced, and `ErrFailedToRecover` was removed.
            *   The `EncryptionResult` struct was updated to remove `FailedToRecover`.
            *   The `EncryptTargetDirs` function was modified to hardcode the `tempDir` path (`"/opt/128technology/integrity/.migration-store/"`) instead of using `common.MigrationDirPath`, passing this `tempDir` as an argument to `encryptTargetDir`. It also added a `switch` statement for logging different failure types and a cleanup step (`fs.RemoveAll(tempDir)`) upon successful encryption.
            *   The `encryptTargetDir` function was refactored: it now accepts `tempDir` as a parameter, and the logic related to `recoveryDir` and the `migrateToRecoveryDir` call was removed or commented out.

### Patterns and Recurring Elements:

*   **Copyright Notice**: All files include the copyright notice `// Copyright (c) Juniper Networks, Inc. 2025. All rights reserved.` at the top.
*   **Security Focus**: The code heavily emphasizes integrity and encryption, dealing with FEMK, secure containers, TPM, and `fscrypt` for robust filesystem protection.
*   **Go Language Constructs**: Consistent use of Go's `context` package for operation cancellation/timeouts, `errors` package for detailed error handling (`errors.Is`, `errors.Join`), and `os` for file mode and path operations.
*   **Filesystem Abstraction**: The `github.com/spf13/afero` library is used for abstracting filesystem operations, making the code more testable and portable.
*   **Logging**: The custom `github.com/Juniper-SSN/ssr/go/src/log` package is used throughout for consistent logging of debug, info, warning, and error messages.
*   **Idempotency Requirement**: Key functions like `enableIntegrity` and `EncryptTargetDirs` are explicitly documented as needing to be idempotent, which is crucial for robust system operations.
*   **Defined Directory Structure**: The application relies on a well-defined set of directories under `/boot/integrity` and `/opt/128technology/integrity` for its operation, migration, and recovery, as outlined in `common/directories.go`.
*   **Error Handling**: A common pattern of returning `(result, error)` and then checking `result.Failed()` or the error itself is prevalent, especially in encryption-related functions.
*   **Module Structure**: The application is organized into several Go packages: `main`, `common`, `fscrypt`, `tpm`, `vault`, indicating a modular design.