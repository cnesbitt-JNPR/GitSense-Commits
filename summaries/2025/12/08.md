...
## 8:26:59 PM
The provided code change log primarily details the evolution of an `IntegrityHandler` application written in Go, focusing on filesystem encryption and integrity management. The changes span three main files: `common/directories.go`, `integrity.go`, and `fscrypt/fscrypt.go`, with a new file `fscrypt/migration.go` introduced later in the log.

**File-specific Updates:**

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/common/directories.go`**
    *   **11/21/2025, 2:26:50 PM**: Initial definition of constants for critical directory paths, including `/boot/integrity`, `/boot/integrity/femk.enc` (for the encrypted File Encryption Master Key), migration and recovery paths under `/opt/128technology/integrity`, and a manifest directory. The package comment initially describes the package as holding secure containers relying on mmap'ed memory for keys.
    *   **11/21/2025, 4:02:40 PM**: The package comment is revised to a simpler description: "Package common contains common globals used throughout Integrity Handler," removing specific mentions of secure containers or mmap'ed memory.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/integrity.go`**
    *   **Initial state (11/21/2025, 2:27:15 PM - 2:40:35 PM)**: This file outlines the main logic of the Integrity Handler. Key functions include `verifyAndInitializeEnvironment` (checking root privileges, kernel version, filesystem encryption support, `fscrypt` command availability, and TPM initialization), `enableIntegrity` (orchestrating directory creation, FEMK resolution, fscrypt setup, and encryption), `unlockEncryptedDirs`, `getFsKey`, and utility functions like `ensureDirectoriesExist` (which handled creating and cleaning migration/recovery directories) and `handleRecoveryDirectoryWarnings`. Error handling distinguishes between general failures and integrity compromises.
    *   **12/1/2025, 4:38:57 PM**: This is a significant refactoring timestamp:
        *   The `os` import is removed.
        *   The `ensureDirectoriesExist` function is streamlined; it no longer includes logic for cleaning the migration directory or handling the recovery directory. Instead, it expects permission constants (`common.MigrationDirPerms`, `common.ManifestDirPerms`, `common.BootDirPerms`) to be defined elsewhere (presumably in `common/directories.go`, though not shown in the log for that file).
        *   A new function, `checkMigrationDirectory`, is introduced to explicitly verify if the migration directory is empty, preventing accidental overwrites of recovery data.
        *   The `handleRecoveryDirectoryWarnings` function is commented out.
        *   Error handling for `fscrypt` command not found is downgraded from returning a critical error to just logging a warning.
    *   **12/1/2025, 4:39:52 PM**: A minor stylistic change in `checkMigrationDirectory` for slice declaration (`var errs []error = ...` to `errs := ...`).
    *   **12/3/2025, 11:57:46 AM**: Enhanced error handling in `enableIntegrity`. It now checks if the `EncryptionResult` indicates a "Compromised" state (implying a new `Compromised()` method on `EncryptionResult`) and if so, explicitly joins `errIntegrityCompromised` with the build error.
    *   **12/3/2025, 2:04:54 PM**: Refinement to the error joining in `enableIntegrity`, switching from `errors.Join` to `fmt.Errorf("%w: %w", errIntegrityCompromised, err)` to correctly wrap the compromised error.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/fscrypt.go`**
    *   **Initial state (11/21/2025, 4:11:30 PM)**: Introduces fscrypt-specific wrappers, defining numerous `Err` constants for various encryption/migration failures. It includes `SetupOnMount` for fscrypt initialization, and `EncryptionResult` struct to track outcomes (success, already encrypted, relocation failures, encryption failures, unlock failures, restore failures, recovery failures). The `encryptTargetDir` function outlines a multi-step process: relocate contents, create/encrypt empty target, unlock, restore contents, and potentially recover.
    *   **11/25/2025, 4:04:34 PM**: A significant structural change:
        *   The `common` package import is removed.
        *   `ErrFailedToEncrypt`, `ErrFailedToUnlock`, `ErrFailedToRestore` error constants are added.
        *   The `EncryptionResult` struct removes `FailedToRecover`, and its `Failed()` and `BuildError()` methods are updated accordingly.
        *   The `encryptTargetDir` function's signature and internal logic are modified to remove direct recovery handling, aligning with the removal of `FailedToRecover`. The temporary directory path (`tempDir`) is hardcoded instead of using `common.MigrationDirPath`.
        *   The `EncryptTargetDirs` function introduces a `switch` statement for detailed error handling after calling `encryptTargetDir`, and explicitly attempts to remove `tempDir` on success.
    *   **12/1/2025, 4:37:06 PM**: Reverts some of the previous changes:
        *   The `common` package import is restored.
        *   `ErrFailedToRecover` is re-added, and `EncryptionResult` is updated to include `FailedToRecover` again.
        *   The `SetupOnMount` function is refactored using a `switch` statement to differentiate global setup (for `""` or `/` mount paths) from filesystem-specific setup.
        *   The `EncryptTargetDirs` function's call to `encryptTargetDir` is modified to receive `tempDir` as a return value, and the error handling `switch` statement is expanded with a `recoveryErr` placeholder.
    *   **12/3/2025, 11:58:10 AM**: Introduces a new `Compromised()` method on the `EncryptionResult` struct, returning `true` if `FailedToUnlock` has any entries. This supports the refined error handling in `integrity.go`.
    *   **12/4/2025, 9:38:27 AM**: Adds `ErrFailedToClean` error constant to handle failures in removing temporary migration directories after successful encryption. The `EncryptionResult` struct, `Failed()` method, and `BuildError()` method are updated to incorporate this new failure type.
    *   **Subsequent timestamps for fscrypt.go (12/1/2025, 4:37:36 PM - 12/5/2025, 4:07:12 PM)**: These mostly show identical code to the last significant change, indicating minor saves or rebuilds without substantial content changes.

*   **`/Users/cnesbitt/shared/ssr/go/bin/IntegrityHandler/fscrypt/migration.go`**
    *   **12/5/2025, 3:57:38 PM**: This new file is introduced, centralizing utility functions for safely moving directory contents. It includes `createNameForTargetDir` (generates a unique hash-based name), `moveIntoDir`, `moveWithDirOverwrite`, `optimizedMove` (using `os.Rename`), and `moveWithRsync` (for robust content transfer using `rsync`). These functions are crucial for the encryption process, which requires temporarily relocating directory contents.
    *   **Subsequent timestamps (12/5/2025, 3:59:27 PM - 4:07:12 PM)**: These are identical to the initial timestamp for this file.

**Timestamps of Significant Changes:**

*   **11/21/2025, 4:02:40 PM**: Simplification of package comment in `common/directories.go`.
*   **12/1/2025, 4:38:57 PM**: Major refactoring in `integrity.go` regarding directory management (`ensureDirectoriesExist` simplification, `checkMigrationDirectory` introduction, `handleRecoveryDirectoryWarnings` removal/commenting).
*   **11/25/2025, 4:04:34 PM**: Substantial structural changes in `fscrypt/fscrypt.go`, including removing `common` import, refining `EncryptionResult`, and altering `encryptTargetDir` logic (some of which were later reverted).
*   **12/1/2025, 4:37:06 PM**: Reintroduction of `common` import and `FailedToRecover` in `fscrypt/fscrypt.go`, along with a refactor of `SetupOnMount` and `EncryptTargetDirs`.
*   **12/3/2025, 11:57:46 AM** (in `integrity.go`) and **12/3/2025, 11:58:10 AM** (in `fscrypt/fscrypt.go`): Introduction of the `Compromised()` method and associated error handling logic to explicitly flag integrity violations.
*   **12/4/2025, 9:38:27 AM**: Addition of `ErrFailedToClean` error and corresponding tracking in `fscrypt/fscrypt.go`.
*   **12/5/2025, 3:57:38 PM**: Introduction of the `fscrypt/migration.go` file, centralizing directory movement utilities.

**Patterns and Recurring Elements:**

*   **Copyright and Package Structure**: All files maintain a `Copyright (c) Juniper Networks, Inc. 2025. All rights reserved.` notice and follow standard Go package definitions (`package common`, `package main`, `package fscrypt`).
*   **Robust Error Handling**: A strong emphasis on detailed error handling is evident throughout, with custom error types (`ErrTargetAlreadyEncrypted`, `ErrFailedToRelocate`, etc.), error wrapping (`fmt.Errorf("%w: %w", ...)`) and joining (`errors.Join`). The concept of "integrity compromised" is a key error state.
*   **Logging**: Consistent use of structured logging (`log.Debug`, `log.Info`, `log.Warnf`, `log.Errorf`) for debugging and operational insights.
*   **Filesystem Abstraction**: The `afero` library is used for abstract filesystem operations, making the code more testable and flexible regarding underlying storage.
*   **Fscrypt Integration**: The core functionality revolves around the `github.com/google/fscrypt` library for managing encrypted directories, including setup, encryption, and unlocking.
*   **Secure Key Management**: References to FEMK (File Encryption Master Key) and `vault.SecureContainer` indicate a focus on secure handling of cryptographic keys.
*   **Idempotency**: Specific functions like `enableIntegrity` and `EncryptTargetDirs` are explicitly designed to be idempotent, meaning they can be run multiple times without causing unintended effects.
*   **Migration and Recovery Logic**: The system includes mechanisms to safely migrate data during encryption and provide recovery options in case of failures, as seen in the temporary migration directories and the `EncryptionResult` tracking various failure states.