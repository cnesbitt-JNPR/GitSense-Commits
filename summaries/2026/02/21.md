...
## 8:02:46 PM
The file `/Users/cnesbitt/shared/ssr/go/bin/ChassisManager/led/heartbeat/monitor.go`, last modified on 2/19/2026 at 5:12:50 PM, defines a Go package named `heartbeat` responsible for periodically checking the health of an SSR (Session Smart Router) system via a `StateMonitor`.

Key features and functionalities include:

*   **Heartbeat Monitoring:** Implements a `Monitor` interface with `Run`, `GetHealth`, and `SetHealth` methods to manage and report the SSR's health status.
*   **Health States:** Defines `SSRHealthResponse` constants (`SSRUnknownHealth`, `SSRHealthy`, `SSRMajorOrCriticalAlarm`) which are mapped to broader `api.SSRHealth` states like `SSRInService`, `SSRDegradedService`, and `SSROutOfService`. `SSRBooting` is an initial state.
*   **Polling Mechanism:** The `Run` method contains a continuous loop that polls the SSR health at a defined `pollInterval` (default 3 seconds). It incorporates initial `bootingThreshold` (100 failures) and `reconnectionThreshold` (3 failures) to manage grace periods during startup or network interruptions.
*   **Unix Socket Communication:** Health checks are performed by making HTTP GET requests to `/api/v1/ssr/health` via a Unix domain socket located at `/var/run/128technology/stateMonitor.sock`. The code uses `unix.NewTransportWithStaticAddress` for this communication.
*   **Response Handling:** The `handleHeartbeatResponse` function parses the HTTP response, checks the status code, and unmarshals a JSON body to determine the specific health status, mapping it to the appropriate `api.SSRHealth` enum.
*   **Error Reporting and Logging:** The monitor logs status changes (in-service, degraded, out-of-service) using `log.Infof`, `log.Warnf`, and `log.Errorf`, providing detailed reasons for service degradation or outages.
*   **Concurrency and Thread Safety:** A `sync.Mutex` (`healthMu`) is used to ensure thread-safe access and modification of the monitor's health status.
*   **Testability:** The design includes a `makeRequest` function pointer and `newTestMonitor` constructor to allow short-circuiting HTTP requests for unit testing purposes.

Overall, the code provides a robust, self-managing system for continuously monitoring and reporting the health of an SSR component, with considerations for startup, recovery, logging, and testability.